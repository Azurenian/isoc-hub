...
searchInput.addEventListener("input", function () {
  ...
  // Recommend only card titles (name) that match the query
  const matches = spreadsheetData
  ...
  if (matches.length > 0) {
    suggestions.innerHTML = matches
      .map(
        title =>
          `<li class="list-group-item list-group-item-action" style="cursor:pointer;">${title}</li>`
      )
      .join("");
    suggestions.style.display = "block";
  } else {
    suggestions.innerHTML = `<li class="list-group-item text-muted">No recommendations</li>`;
    suggestions.style.display = "block";
  }
